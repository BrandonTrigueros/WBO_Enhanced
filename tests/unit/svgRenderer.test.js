...
describe("svgRenderer", function () {
  describe("toSVG", function () {
    ...
    it("should render a Rectangle as <rect>", async function () {
      ...
      assert.ok(
        svg.includes('stroke-width="3"'),
        "should have correct stroke width",
      );
    });
    ...
    it("should render Pencil paths from _children", async function () {
      ...
      assert.ok(
        svg.includes('stroke="#00f"'),
        "should have correct stroke color",
      );
    });
    ...
    it("should render Straight line as a path", async function () {
      ...
      assert.ok(svg.includes("<path"), "should contain <path>");
      assert.ok(
        svg.includes("M10 10L200 150"),
        "should have correct line path",
      );
    });
    ...
    it("should handle transform (deltax, deltay)", async function () {
      ...
      var svg = await renderToString(board);
      assert.ok(
        svg.includes('transform="translate(50,30)"'),
    ...
    it("should skip unknown tools without crashing", async function () {
      ...
      assert.ok(
        !svg.includes("UnknownTool"),
        "should not render unknown tool as element",
      );
    });
    ...
    it("should render Image element as <image> tag", async function () {
      ...
      assert.ok(
        svg.includes("/images/test/abc123.png"),
        "should include src path",
      );
    });
    ...
    it("should include Image dimensions in bounding box", async function () {
      ...
      assert.ok(
        parseInt(widthMatch[1]) >= 1200,
        "width should account for image dimensions",
      );
    });
  });
});
