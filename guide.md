...
![wavy bezier curve path drawing](image)
...
![Wavy SVG path illustration showing a smooth bezier-like curve across the page](image)
...
![Triangle drawn by doc.polygon([100, 0], [50, 100], [150, 100])](image)
...
![dashed circle with evenly spaced dashes](image)
...
![Red filled circle with dark red stroke](page_21_image_1_v2.jpg)
...
![Square filled with a green-to-red linear gradient](page_22_image_1_v2.jpg)

![Circle filled with an orange radial gradient](page_22_image_2_v2.jpg)
...
![Two red five-point stars: left is a filled star, right is a star showing an inner hole demonstrating even-odd winding rule](page_23_image_1_v2.jpg)
...
![Circular checkerboard pattern clipped to a circle](page_25_image_1_v2.jpg)
...
If you're working with documents that require precise layout, you may need to know the size of a piece of text. PDFKit has three methods to achieve this: `widthOfString(text, options)`, `heightOfString(text, options)` and `boundsOfString(text, options)`/`boundsOfString(text, x, y, options)`. All methods use the same options described in the Text styling section, and take into account the eventual line wrapping.
...
Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam in suscipit purus. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Vivamus nec hendrerit felis. Morbi aliquam facilisis risus eu lacinia. Sed eu leo in turpis fringilla hendrerit.hendrerit felis. Morbi aliquam facilisis risus eu lacinia. Sed eu leo in turpis fringilla hendrerit. Ut nec accumsan nisl. Suspendisse rhoncus nisl posuere tortor tempus et dapibus elit porta. Cras leo neque, elementum
...
![Small cropped view (Centered)](page_38_image_4_v2.jpg)

**Stretch**

![Wide stretched view (Stretch)](page_38_image_2_v2.jpg)

**Scale**

![Scaled smaller view (Scale)](page_38_image_3_v2.jpg)
...
Many of the annotations have a **color** option that you can specify. You can use an array of RGB values, a hex color, a named CSS color value, or a named spot color value for that option.

A custom icon can be set using option **Name** property. Possible values are: 'Comment', 'Key', 'Note', 'Help', 'NewParagraph', 'Paragraph' and 'Insert'.
...
![yellow note icon](image)

<mark>This text is highlighted!</mark>

![yellow note icon](image)

This text uses spot color!
...
Form Annotations are, by default, added to the root of the PDF document. A PDF form is organized in a name heirarchy, for example shipping.address.street. Capture this heirarchy either by setting the **name** of each form annotation with the full hierarchical name (e.g. shipping.address.street) or by creating a hierarchy of form fields and form annotations and refering to a form field or form annotations parent using **options.parent**. A form field is an invisible node in the PDF form and is created using the document **formField** method. A form field must include the node's name (e.g. **shipping**) and may include other information such as the default font that is to be used by all child form annotations. Using the formField method you might create a shipping field that is added to the root of the document, an address field that refers to the shipping field as it's parent, and a street Form Annotation that would refer to the address field as it's parent. Create form fields using the document method: `formField( name, options )` - returns a reference to the field
...
In support of Form Annotations that execute JavaScript in PDF, you may use the following
...
PDFKit makes adding tables to documents quite simple, and includes many options to customize the display of the output.
...
<table>
  ...
  <tbody>
    <tr>
        <td>One value goes here</td>
<td>Another one here</td>
<td>OK?</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
        <td>fixed-width cells have<br/>exactly the specified width</td>
<td>nothing interesting here</td>
<td>nothing interesting here</td>
<td>nothing interesting here</td>
    </tr>
  </tbody>
...
```
// doc.table example from the page
doc.table({
  rowStyles: [20, 50, 70],
  data: [
...
<table>
  <tbody>
    <tr>
        <td>row 1 with height 20</td>
<td>column B</td>
    </tr>
<tr>
        <td>row 2 with height 50</td>
<td>column B</td>
    </tr>
<tr>
        <td>row 3 with height 70</td>
<td>column B</td>
    </tr>
  </tbody>
</table>
...
<table>
  <thead>
    <tr>
        <th colspan="2">Header with Colspan = 2</th>
        <th>Header 3</th>
    </tr>
<tr>
        <th>Header 1</th>
...
<tr>
        <td rowspan="3">rowspan set to 3<br/>Lorem ipsum dolor sit amet, consectetur<br/>adipisicing elit, sed do eiusmod tempor</td>
<td>Sample value 2</td>
<td>Sample value 3</td>
    </tr>
<tr>
...
<tr>
        <td>Sample value 1</td>
        <td colspan="2" rowspan="2">Both:<br/>rowspan and colspan<br/>can be defined at the same time</td>
    </tr>
<tr>
        <td>Sample value 1</td>
    </tr>
...
```
// doc.table example from screenshot
doc.table({
data:    [
  [
...
\n[true,  false,  false,  false]" },
  ...
  [
    {    rowSpan:  3, border:  true, backgroundColor:   "#eef",   text: "rowSpan:
3\n\nborder:\ntrue" },
...
\n[false, false,   false,  true]" },
  ...
  [
    {    colSpan:  2, border:  true,     backgroundColor: "#efe", text: "colSpan:
2\n\nborder:\ntrue" },
  ],
  [
...
rowSpan: 3                      border:                 border:
                                undefined (default)     [false, false, false, true]
border:                         colSpan: 2
true
                                border:
                                true
                                border:                 border:
                                0 (same as false)       [false, true, true, false]
```

<table>
  <tbody>
    <tr>
        <td>border: [true, false, false, false]</td>
<td>border: false</td>
<td>border: true</td>
    </tr>
<tr>
        <td rowspan="3">rowSpan: 3<br/>border: true</td>
<td>border: undefined (default)</td>
<td>[false, false, false, true]</td>
    </tr>
<tr>
        <td colspan="2">colSpan: 2<br/>border: true</td>
    </tr>
<tr>
        <td>border: 0 (same as false)</td>
...
<table>
  ...
  <tbody>
    <tr>
        <td>row 1</td>
        <td colspan="3" rowspan="3">rowSpan: 3<br/>colSpan: 3<br/><br/>border:<br/>[true, true, true, true]</td>
    </tr>
<tr>
        <td>row 2</td>
    </tr>
